...
import { useTheme, type ThemeContextValue } from '../src/providers/ThemeProvider';
...
export default function JournalScreen() {
  ...
  async function handleGenerate() {
    try {
      ...
      Alert.alert(
        '일지 생성 실패',
        '탐험 일지를 만들지 못했어요. 오늘 탐험 기록이 있는지 확인해 주세요.',
        [{ text: '확인' }],
      );
    } finally {
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top, backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { borderBottomColor: colors.border, backgroundColor: colors.surface }]}>
        <Pressable style={[styles.backBtn, { backgroundColor: colors.surfaceLight }]} onPress={() => router.back()} hitSlop={12}>
          <Ionicons name="chevron-back" size={24} color={colors.textPrimary} />
        </Pressable>
        <Text style={[styles.headerTitle, { color: colors.textPrimary }]}>탐험 일지</Text>
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(300)} style={styles.monthNav}>
          <Pressable onPress={() => navigateMonth(-1)} hitSlop={12} style={[styles.navArrow, { backgroundColor: colors.surface }]}>
        ...
        <View style={styles.dayLabelsRow}>
          {DAY_LABELS.map((label) => (
            <View key={label} style={styles.dayLabelCell}>
              <Text style={[styles.dayLabelText, { color: colors.textMuted }]}>{label}</Text>
            </View>
          ))}
        </View>
        ...
        <Animated.View entering={FadeIn.duration(400)} style={styles.heatmapGrid}>
          {monthCells.map((day, idx) => {
            if (day === null) {
              return <View key={`empty-${idx}`} style={[styles.heatCell, { backgroundColor: colors.surface }]} />;
            }
        ...
        <View style={styles.selectedSection}>
          {isLoadingDay ? (
            <View style={styles.skeletonContainer}>
              <View style={[styles.skeletonBubble, { backgroundColor: colors.surface }]}>
            ...
            <Animated.View entering={FadeInUp.duration(300)} key={selectedJournal.id}>
              {/* Diary Page Card */}
              <View style={[styles.diaryPage, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                {/* Diary Date Header */}
                <View style={[styles.diaryDateHeader, { borderBottomColor: colors.border }]}>
                  <View style={styles.diaryDateLeft}>
                    <Text style={[styles.diaryDateDay, { color: colors.textPrimary }]}>
                      {selectedDate.split('-')[2]}
                    </Text>
                    <View>
                      <Text style={[styles.diaryDateDow, { color: BRAND.primary }]}>
                        {selectedDayOfWeek}요일
                      </Text>
                      <Text style={[styles.diaryDateFull, { color: colors.textMuted }]}>
                        {selectedDate.replace(/-/g, '. ')}
                      </Text>
                    </View>
                  </View>
                  <View style={[styles.diaryCharacterBadge, { backgroundColor: `${BRAND.primary}15` }]}>
                    <CharacterAvatar
                    ...
                    />
                    <Text style={[styles.diaryCharacterName, { color: colors.textSecondary }]}>{characterName}</Text>
                  </View>
                </View>

                {/* Diary Body */}
                <View style={styles.diaryBody}>
                  <Text style={[styles.diaryText, { color: colors.textPrimary }]}>
                ...
                {(selectedJournal.share_card?.places_visited ?? []).length > 0 && (
                  <View style={[styles.diaryPlaces, { borderTopColor: colors.border }]}>
                    <View style={styles.placesSectionHeader}>
                      <Ionicons name="location-outline" size={14} color={colors.textSecondary} />
                      <Text style={[styles.placesSectionTitle, { color: colors.textSecondary }]}>방문한 장소</Text>
                    </View>
                    <ScrollView
                      horizontal
                    ...
                    >
                      {selectedJournal.share_card.places_visited.map((place, i) => (
                        <View key={i} style={[styles.placeChip, { backgroundColor: colors.surfaceLight, borderColor: colors.border }]}>
                          <Ionicons name="pin" size={12} color={BRAND.primary} />
                          <Text style={[styles.placeChipText, { color: colors.textPrimary }]}>{place}</Text>
                        </View>
                      ))}
                    </ScrollView>
                  </View>
                ...
                {/* Stats Stickers */}
                <View style={[styles.diaryStats, { borderTopColor: colors.border }]}>
                  <View style={[styles.diaryStat, { backgroundColor: '#FBBF2415' }]}>
                    <Ionicons name="flash" size={16} color="#FBBF24" />
                    <Text style={[styles.diaryStatValue, { color: colors.textPrimary }]}>
                      +{selectedJournal.share_card?.xp_earned ?? 0}
                    </Text>
                    <Text style={[styles.diaryStatLabel, { color: colors.textMuted }]}>XP</Text>
                  </View>
                  <View style={[styles.diaryStat, { backgroundColor: '#F472B615' }]}>
                    <Ionicons name="ribbon" size={16} color="#F472B6" />
                    <Text style={[styles.diaryStatValue, { color: colors.textPrimary }]}>
                      {(selectedJournal.share_card?.badges_earned ?? []).length}
                    </Text>
                    <Text style={[styles.diaryStatLabel, { color: colors.textMuted }]}>배지</Text>
                  </View>
                  <View style={[styles.diaryStat, { backgroundColor: `${BRAND.primary}15` }]}>
                    <Ionicons name="footsteps-outline" size={16} color={BRAND.primary} />
                    <Text style={[styles.diaryStatValue, { color: colors.textPrimary }]}>
                      {(selectedJournal.share_card?.places_visited ?? []).length}
                    </Text>
                    <Text style={[styles.diaryStatLabel, { color: colors.textMuted }]}>장소</Text>
                  </View>
                </View>
              </View>
...
function getStyles(colors: ThemeContextValue['colors']) {
  ...
  diaryText: {
    ...
    fontFamily: FONT_FAMILY.narrative,
  },
  diaryPlaces: {
    paddingHorizontal: SPACING.lg,
    paddingVertical: SPACING.md,
