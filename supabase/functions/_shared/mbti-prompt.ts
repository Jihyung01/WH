/**
 * MBTI 성격 수정자 — Edge Function 공용 헬퍼.
 *
 * 클라이언트의 src/data/mbti-personality-modifiers.ts와 "의미적으로 동일"한
 * 데이터를 담고 있다. Deno Edge Function에서는 src/ 경로를 import할 수 없기
 * 때문에 이 파일에 복제한다. 두 파일을 수정할 때는 항상 같이 갱신한다.
 *
 * 🚨 규칙
 *   1. 이 파일의 내용은 system prompt 내부 설정값으로만 사용된다.
 *   2. 사용자에게 MBTI 코드나 수정자 내용이 절대 그대로 노출되어선 안 된다.
 *   3. 기본 캐릭터 페르소나 위에 "얹는 편향"으로 주입한다.
 */
...
const MBTI_REGEX = /^[EI][NS][TF][JP]$/;

export function isValidMBTI(code: string | null | undefined): boolean {
  if (!code) return false;
  return MBTI_REGEX.test(code.toUpperCase());
}

export const MBTI_MODIFIERS: Record<string, MBTIModifier> = {
  INTJ: {
    code: "INTJ",
    label: "전략적인 사색가",
    tone:
  ...
  ISTJ: {
    code: "ISTJ",
    label: "청렴결백한 논리주의자",
    tone: "차분하고 사실 기반으로 말한다. 수식어가 적고, 정확한 용어를 선호한다.",
  ...
  ISTP: {
    code: "ISTP",
    label: "냉철한 장인",
    tone: "과묵하고 실용적으로 말한다. 쓸데없는 수식어를 안 쓴다. 핵심만.",
  ...
  ISFP: {
    code: "ISFP",
    label: "자유로운 예술가",
    tone: "조용하고 감각적으로 말한다. 보고 듣고 느끼는 것에 집중한다.",
  ...
  ESFP: {
    code: "ESFP",
    label: "자유로운 연예인",
    tone: "밝고 유쾌하게 말한다. 현재 순간을 즐기고, 재미를 최우선으로 한다.",
...
export function getModifier(
  ...
  return MBTI_MODIFIERS[(mbti as string).toUpperCase()] ?? null;
}

const DENSITY_HINT: Record<EmojiDensity, string> = {
  none: "이모지는 사용하지 않는다.",
  low: "이모지는 꼭 필요할 때만 최대 1개까지만 사용한다.",
  medium: "이모지는 문장당 최대 1~2개까지 적절히 사용한다.",
};

const LENGTH_HINT: Record<SentenceLength, string> = {
...
/**
 * Edge Function system prompt의 말미에 추가할 "내부 성격 수정자" 블록.
 * MBTI가 null/unknown이면 빈 문자열을 반환한다 (수정자 미적용).
 *
 * 이 블록은 system prompt의 맨 마지막에 붙이는 것을 권장한다 — 기본 캐릭터
 * 페르소나·안전 가드·규칙이 먼저 선언된 뒤 톤 편향이 "얹어지는" 구조.
 */
export function buildMBTIPromptAddendum(
  ...
  return [
    ...
    `- 말투 길이: ${LENGTH_HINT[mod.sentence_length]}`,
    `- 이모지 정책: ${DENSITY_HINT[mod.emoji_density]}`,
...
/**
 * profiles.mbti 컬럼을 안전하게 읽어온다.
 * - 인증된 클라이언트(service role)가 필요하다.
 * - 실패 또는 미설정 시 null 반환.
 */
export async function fetchUserMBTI(
  supabase: {
    from: (table: string) => {
      select: (cols: string) => {
        eq: (col: string, val: string) => {
          maybeSingle: () => Promise<
            { data: { mbti: string | null } | null; error: unknown }
          >;
        };
      };
    };
  ...
  try {
    ...
    return isValidMBTI(mbti) ? (mbti as string).toUpperCase() : null;
  } catch {
    return null;
  }
}
