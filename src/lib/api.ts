...
export type JournalFormat = 'narrative' | 'timeline';
...
export interface JournalTimelineData {
  intro: string;
...
export interface JournalEntry {
  ...
  created_at: string;
  /** 'narrative' (legacy) or 'timeline' (흔적 기반). Older rows default to 'narrative'. */
  format?: JournalFormat;
  /** Timeline payload (structured). null/undefined for narrative journals. */
  timeline_data?: JournalTimelineData | null;
  /** marks.id[] — chronological order. */
  mark_ids?: string[];
  /** Visited districts (deduped). */
  districts?: string[];
  /** true when auto-generated from mark threshold. */
...
/**
 * Generate (or fetch cached) journal for a given date.
 * `options.autoGenerated=true` 는 흔적 ≥3 자동 트리거 플로우에서만 전달.
 */
export async function generateJournal(
  ...
  return invokeEdgeFunction<GenerateJournalResult>('generate-journal', {
    ...
    auto_generated: options?.autoGenerated === true,
  });
}
...
// ─────────────────────────────────────────────────────────────────────────────
// 27. Marks (흔적) — 위치 기반 경량 콘텐츠 + 자동 일지 트리거
//     Backend RPC: create_mark / get_nearby_marks / get_my_marks / get_today_mark_count
// ─────────────────────────────────────────────────────────────────────────────

/**
 * RPC 행 → Mark 객체 정규화.
 * RPC는 lat/lng를 분리 반환하지만 TypeScript Mark는 `location: {lat, lng}` 구조.
 */
function rowToMark(row: Record<string, unknown>): Mark {
  const lat = typeof row.lat === 'number' ? row.lat : Number(row.lat ?? 0);
  const lng = typeof row.lng === 'number' ? row.lng : Number(row.lng ?? 0);
  const vis = (row.visibility as MarkVisibility) ?? 'public';
  return {
    id: String(row.id ?? ''),
    ...
    music_json: (row.music_json as MarkMusicAttachment | null) ?? null,
    emoji_icon: String(row.emoji_icon ?? '📍'),
    visibility: vis,
    expires_at: (row.expires_at as string | null) ?? null,
    created_at: String(row.created_at ?? ''),
    username: (row.username as string | null | undefined) ?? undefined,
    character_emoji: (row.character_emoji as string | null | undefined) ?? undefined,
    character_class: (row.character_class as string | null | undefined) ?? undefined,
  };
}

function parseMarkRows(raw: unknown): Mark[] {
  if (raw == null) return [];
  let arr: unknown[] = [];
...
/** Supabase Storage `marks-photos` 버킷에 흔적 사진 업로드 (create_mark 전에 호출) */
export async function uploadMarkPhoto(imageUri: string): Promise<string> {
  ...
  const uploadUrl = `${SUPABASE_URL.replace(/\/$/, '')}/storage/v1/object/marks-photos/${fileName}`;
  const res = await fetch(uploadUrl, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${token}`,
  ...
  const {
    data: { publicUrl },
  } = supabase.storage.from('marks-photos').getPublicUrl(fileName);

  return publicUrl;
}

/** 흔적 생성 (RPC `create_mark`). `params.photo_url`은 사전 업로드된 Storage URL이어야 한다. */
export async function createMark(params: CreateMarkParams): Promise<CreateMarkResult> {
  await getCurrentUser();
  const { data, error } = await supabase.rpc('create_mark', {
    p_content: params.content,
    p_photo_url: params.photo_url,
    p_lat: params.lat,
    p_lng: params.lng,
    p_district: params.district ?? null,
    p_music_json: params.music_json ?? null,
    p_visibility: params.visibility ?? 'public',
    p_expires_at: params.expires_at ?? null,
    p_emoji_icon: params.emoji_icon ?? '📍',
  });
  throwIfError(error, '흔적을 남기지 못했습니다.');

  const raw = (data ?? {}) as Record<string, unknown>;
  const markRaw = (raw.mark ?? {}) as Record<string, unknown>;

  return {
    mark: rowToMark(markRaw),
    xp: typeof raw.xp === 'number' ? raw.xp : Number(raw.xp ?? 0),
    level: typeof raw.level === 'number' ? raw.level : Number(raw.level ?? 1),
    today_mark_count:
      typeof raw.today_mark_count === 'number'
        ? raw.today_mark_count
        : Number(raw.today_mark_count ?? 0),
    should_generate_journal: raw.should_generate_journal === true,
  };
}

/** 지도 중심 기준 주변 흔적 조회 (RPC `get_nearby_marks`, 반경 km, 기본 2km) */
export async function getNearbyMarks(
  lat: number,
  lng: number,
  radiusKm = 2,
): Promise<Mark[]> {
  const { data, error } = await supabase.rpc('get_nearby_marks', {
    p_lat: lat,
    p_lng: lng,
    p_radius_km: radiusKm,
    p_limit: 50,
  });
  throwIfError(error, '주변 흔적을 불러오지 못했습니다.');
  return parseMarkRows(data as unknown);
}

/** 특정 날짜의 내 흔적 타임라인 (기본: 오늘). 탐험 일지 구성용. */
export async function getMyMarks(date?: string): Promise<Mark[]> {
  await getCurrentUser();
  const { data, error } = await supabase.rpc('get_my_marks', {
    p_date: date ?? null,
    p_limit: 50,
  });
  throwIfError(error, '내 흔적을 불러오지 못했습니다.');
  return parseMarkRows(data as unknown);
}

/** 오늘 내가 남긴 흔적 개수 (일지 자동 생성 판단용). */
export async function getTodayMarkCount(): Promise<number> {
  await getCurrentUser();
  const { data, error } = await supabase.rpc('get_today_mark_count');
  throwIfError(error, '오늘의 흔적 수를 불러오지 못했습니다.');
  return typeof data === 'number' ? data : Number(data ?? 0);
}

/** Edge Function `generate-journal` 호출 — 오늘 흔적 모음으로 서사형 일지 생성 */
export async function triggerGenerateJournalFromMarks(): Promise<{
  success: boolean;
  journal_id?: string;
}> {
  return invokeEdgeFunction<{ success: boolean; journal_id?: string }>(
    'generate-journal',
    { source: 'marks' },
  );
}
