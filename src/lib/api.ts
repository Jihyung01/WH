...
// ─────────────────────────────────────────────────────────────────────────────
// Marks (흔적) — 20260417120100_create_marks_rpcs.sql
// ─────────────────────────────────────────────────────────────────────────────

function parseMarkMusic(raw: unknown): MarkMusicAttachment | null {
  if (!raw || typeof raw !== 'object') return null;
  const obj = raw as Record<string, unknown>;
  const appleSongId =
    typeof obj.apple_song_id === 'string' && obj.apple_song_id.length > 0 ? obj.apple_song_id : null;
  const title = typeof obj.title === 'string' ? obj.title : null;
  const artist = typeof obj.artist === 'string' ? obj.artist : null;
  if (!appleSongId || !title || !artist) return null;
  return {
    apple_song_id: appleSongId,
    title,
    artist,
    artwork_url: typeof obj.artwork_url === 'string' ? obj.artwork_url : null,
    preview_url: typeof obj.preview_url === 'string' ? obj.preview_url : null,
    apple_music_url: typeof obj.apple_music_url === 'string' ? obj.apple_music_url : null,
  };
}

function rowToMark(raw: unknown, fallbackUserId?: string | null): Mark | null {
  if (!raw || typeof raw !== 'object') return null;
  ...
  const visibilityRaw = typeof r.visibility === 'string' ? r.visibility.toLowerCase() : 'public';
  const visibility: MarkVisibility =
    visibilityRaw === 'friends' || visibilityRaw === 'private'
      ? (visibilityRaw as MarkVisibility)
      : 'public';
  ...
  return {
    ...
    music_json: parseMarkMusic(r.music_json),
    emoji_icon: typeof r.emoji_icon === 'string' && r.emoji_icon.length > 0 ? r.emoji_icon : '📍',
    visibility,
    expires_at: typeof r.expires_at === 'string' ? r.expires_at : null,
    created_at: typeof r.created_at === 'string' ? r.created_at : new Date().toISOString(),
    username: typeof r.username === 'string' ? r.username : null,
    character_emoji: typeof r.character_emoji === 'string' ? r.character_emoji : null,
    character_class: typeof r.character_class === 'string' ? r.character_class : null,
  };
}

function parseMarksPayload(raw: unknown, fallbackUserId?: string | null): Mark[] {
  if (!Array.isArray(raw)) return [];
  const out: Mark[] = [];
  for (const row of raw) {
    const m = rowToMark(row, fallbackUserId);
    if (m) out.push(m);
  }
  return out;
}
...
export async function getNearbyMarks(
  ...
  if (error) throw new AppError(error.message, 'NEARBY_MARKS_FAILED');
  return parseMarksPayload(data);
}

export async function getMyMarks(date?: string, limit = 50): Promise<Mark[]> {
  const user = await getCurrentUser();
  const { data, error } = await supabase.rpc('get_my_marks', {
    p_date: date ?? null,
    p_limit: limit,
  });
  if (error) throw new AppError(error.message, 'MY_MARKS_FAILED');
  return parseMarksPayload(data, user.id);
}

export async function getTodayMarkCount(): Promise<number> {
  const { data, error } = await supabase.rpc('get_today_mark_count');
  if (error) throw new AppError(error.message, 'TODAY_MARK_COUNT_FAILED');
  const n = typeof data === 'number' ? data : Number(data);
  return Number.isFinite(n) ? n : 0;
}

/**
 * `create_mark` RPC 래퍼. 사진 업로드는 호출 측에서 `uploadMarkPhoto`로 선행한 뒤
 * `params.photo_url`에 공개 URL을 담아 전달한다.
 */
export async function createMark(params: CreateMarkParams): Promise<CreateMarkResult> {
  ...
  if (error) throw new AppError(error.message, 'CREATE_MARK_FAILED');

  const payload = (data ?? {}) as Record<string, unknown>;
  const markRow = rowToMark(payload.mark, user.id);
  if (!markRow) {
    throw new AppError('흔적 생성 응답이 유효하지 않습니다.', 'CREATE_MARK_INVALID_RESPONSE');
  }

  const todayCountRaw = payload.today_mark_count;
  const xpRaw = payload.xp;
  const levelRaw = payload.level;

  return {
    mark: markRow,
    xp: typeof xpRaw === 'number' ? xpRaw : Number(xpRaw) || 0,
    level: typeof levelRaw === 'number' ? levelRaw : Number(levelRaw) || 1,
    today_mark_count:
      typeof todayCountRaw === 'number' ? todayCountRaw : Number(todayCountRaw) || 0,
